...
import { Sparkles, Fish, Users, Rocket, Zap, Loader2, type LucideIcon } from "lucide-react";

interface BadgeItemProps {
  type: "SHARK" | "PAIR" | "QUICK" | "YOLO";
  title: string;
  desc: string;
  icon: LucideIcon;
  colorClass: string;
  ...
  loading: string | null;
  success: string | null;
  onUnlock: (type: "SHARK" | "PAIR" | "QUICK" | "YOLO") => void;
}

function BadgeItem({ type, title, desc, icon: Icon, colorClass, btnText, loading, success, onUnlock }: BadgeItemProps) {
  return (
  ...
  return (
    <div className="glass rounded-[40px] p-10 h-fit mt-8 relative overflow-hidden transition-all duration-700">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <BadgeItem type="SHARK" title="Pull Shark" desc="Automated PR lifecycle fulfillment for merged contribution stats." icon={Fish} colorClass="bg-emerald-500/10 text-emerald-400" btnText="Initiate" loading={loading} success={success} onUnlock={unlockBadge} />
        <BadgeItem type="PAIR" title="Pair Programmer" desc="Collaborative attribution with co-authored commit signatures." icon={Users} colorClass="bg-emerald-500/10 text-emerald-400" btnText="Engage" loading={loading} success={success} onUnlock={unlockBadge} />
        <BadgeItem type="QUICK" title="Quick Draw" desc="Record-speed issue resolution for high-velocity response stats." icon={Zap} colorClass="bg-emerald-500/10 text-emerald-400" btnText="Deploy" loading={loading} success={success} onUnlock={unlockBadge} />
        <BadgeItem type="YOLO" title="YOLO" desc="Direct branch merge protocol bypassing external review gates." icon={Rocket} colorClass="bg-emerald-500/10 text-emerald-400" btnText="Launch" loading={loading} success={success} onUnlock={unlockBadge} />
      </div>
    </div>
  );
}
